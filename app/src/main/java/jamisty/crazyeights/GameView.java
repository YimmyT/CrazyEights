...
/**
 * Created by J. Evans on 8/22/2017.
 */
...
public class GameView extends View {
    ...
    protected void onDraw(Canvas canvas) {
        ...
        // Draw the draw/deck and discard pile (if there cards there)
        ...
        // Draw player hand and/or and moving cards
        if (myHand.size() > 7) {
    ...
    private void showChooseSuiteDialog() {
    ...
    private void makeComputerPlay(){
        ...
        //  if the computer drew cards, tell user. Otherwise, skip
        if (computerDrawCount == 1) {
            //  tell user. if count is more than one, please add an 's' to card.
            Toast computerDrawToast = Toast.makeText(myContext, "Computer drew a card!", Toast.LENGTH_SHORT);
            computerDrawToast.setGravity(Gravity.TOP|Gravity.CENTER_HORIZONTAL, 0, 0);
            computerDrawToast.show();
        } else if (computerDrawCount > 1) {
            Toast computerDrawToast = Toast.makeText(myContext, "Computer drew " + computerDrawCount +" cards!", Toast.LENGTH_SHORT);
            computerDrawToast.setGravity(Gravity.TOP|Gravity.CENTER_HORIZONTAL, 0, 0);
            computerDrawToast.show();
        }
    ...
    private void endHand() {
        ...
        if (myHand.isEmpty()) {
            if (myScore >= scoreToWin) {
                ...
                endHandText.setText("You went and got " + scoreThisHand + " points!");
            }
        } else if (oppHand.isEmpty()) {
            if (oppScore >= scoreToWin) {
                endHandText.setText("The computer reached " + oppScore + " points. You lose! Would you like to play again?");
            } else {
        ...
        if (oppScore >= scoreToWin || myScore >= scoreToWin) {
